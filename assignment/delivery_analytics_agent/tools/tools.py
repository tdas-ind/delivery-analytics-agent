...
# Global variables to store dataframes (workaround for ADK limitation)
_delivery_df = None
_payout_df = None
...
# Global cached model for judge (to reuse cached context)
...
def judge_pandas_code(user_query: str, pandas_code: str, data_context_json: str) -> Dict[str, Any]:
    ...
    try:
        # Parse the data context from JSON string
        data_context = json.loads(data_context_json)
        
        # Define static system context (what we want to cache)
        SYSTEM_CONTEXT = """
        ...
        # Create dynamic evaluation prompt (this changes each time)
        evaluation_prompt = f"""
        ...
        # Simple logic: if cached model exists, use it; else create and cache it
        if _cached_judge_model is None:
            # Create new cached model with system context
        ...
        # Use the cached model (whether newly created or existing)
...
def generate_pandas_code(user_query: str, data_context_json: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for iteration in range(max_iterations):
            # Create code generation prompt (include previous feedback if available)
            ...
            # If validation passed, return the successful code
        ...
        # After max iterations, return the latest code even if not approved
        return {
            'pandas_code': current_code,
...
# Helper function to generate mathematical breakdown of payout calculation (UPDATED)
def _calculate_mathematical_breakdown(delivery_data: pd.Series, payout_rules: List[Dict]) -> str:
    ...
    try:
        ...
        # Extract delivery partner information
...
# Tool 4: Execute generated pandas code safely with mathematical breakdown
def execute_pandas_code(pandas_code: str) -> Dict[str, Any]:
    ...
    try:
        # Validate dataframes are loaded
        ...
        if isinstance(result, pd.DataFrame):
            ...
            # Generate mathematical breakdown for payout-related calculations
            if any(col.lower() in ['payout', 'total', 'earn', 'amount', 'salary'] for col in result.columns):
                ...
                for idx, row in result.iterrows():
                    if idx >= 3:  # Limit detailed breakdown to first 3 entries
                        break
                    
                    name = row.get('Name', f'Record {idx+1}')
                    
                    # Find corresponding delivery data for mathematical breakdown
                    ...
                    # Add calculation summary for each result
            ...
            # Add mathematical context for series results
...
# Tool 5: Professional response formatting with mathematical explanations
def format_response_with_calculations(query_result_json: str, user_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result_data['type'] == 'dataframe' and result_data['data']:
            ...
            # Identify calculation methodology based on columns present
...
def analyze_delivery_data(csv_path: str, user_query: str) -> Dict[str, Any]:
    ...
    """
    try:
        # Step 1: Load and prepare data with schema detection
        ...
        # Step 2: Generate validated pandas code with iterative improvement
        ...
        # Step 3: Execute pandas code with mathematical breakdown
        ...
        # Step 4: Format response with professional mathematical explanations
...
# Initialize the professional delivery analytics processor agent
